...
function send_mail($uemail,$name,$token)
{
    ...
    $email->setFrom("ak6726084@gmail.com", "Booking");
    $email->setSubject("Account Verification");
    $email->addTo($uemail, $name);
     $email->addContent(
        "text/html",
        "Click the link to confirm email: <br>
        <a href='".SITE_URL."email_confirm.php?email=$uemail&$token"."'>
        CLICK ME 
        </a>"
    );
...
if (isset($_POST['register'])) {
    ...
    $values = [$data['name'], $data['email'], $data['address'], $data['phonenum'], $data['pincode'], $data['dob'],$enc_pass,$img,$token];
